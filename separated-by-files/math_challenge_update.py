...
operations_list = len(list(operations_type.keys()))
...
def ensure_option_menu_correctly(user_input,message_input,options_available=list): #ensure allowed input
    options_available_to_use = ('/'.join (i for i in options_available))
    while user_input not in options_available:
        print(f"This option is not available in the menu, please choose one option among them({options_available_to_use}) ")
...
""")   
    ...
    calculation_complexity = int(input(message_input_calc_complex))
    ensure_option_menu_correctly(calculation_complexity, message_input_calc_complex, [1,2])
    print("""1 - Easy
2 - Medium
3 - Hard
4 - Very Hard
""")
    ...
    level_difficult = input(level_difficult_message)
    if level_difficult not in levels:
        level_difficult = input("Choose a number between 1 and 4, please: ")
...
op_index = {
    'easy': (0,4),
    'medium': (2,4),
    'hard': (2, operations_list),
    'very hard': (2, operations_list)
}
